...
#define NORMAL_ACCOUNTS "./ACCOUNTS/NORMAL/Normalaccount.bank"
#define JOINT_ACCOUNTS "./ACCOUNTS/JOINT/Jointaccount.bank"
#define CUSTOMERS "./CUSTOMERs/customer.bank"
...
bool login_handler(bool isAdmin, int connFD, struct Customer *ptrToCustomerID)
{
    ...
    if (isAdmin)
        ...
        strcpy(writeBuffer, "Welcome dear customer! Enter your credentials ");
...
bool get_Joint_account_details(int connFD, struct Joint_Account *customerAccount)
{
    ...
    if (accountFileDescriptor == -1)
    {
        // Account record doesn't exist
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "Enter the account number of the account you're searching for");
        strcat(writeBuffer, "^");
        perror("Error opening account file in get_account_details!");
        writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
        if (writeBytes == -1)
